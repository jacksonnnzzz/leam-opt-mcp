...
当前活动共包含 **9 个参考设计**：1 个 Ansys 官方示例，以及 5 篇开放论文拆分出的
8 个论文设计。9 个参考设计均已在本地 AEDT 2025.1/PyAEDT 0.26.3 中完成真实求解。

| 指标 | 当前结果 | 可得出的结论 |
| --- | ---: | --- |
| 已完成论文/官方来源审计 | 9/9 | 参数、图片拓扑和响应目标已经冻结 |
| 已完成确定性建模脚本与离线测试 | 9/9 | 代码可以复查，缺失信息不会被冒充为论文参数 |
| 已完成 HFSS 参考求解 | 9/9 | 每个参考设计都有真实 AEDT 求解证据 |
| 已完成 HFSS 参考求解并通过自身门槛 | 1/9 | 只有官方探针馈电贴片具备本地电磁基线证据 |
| 已完成 HFSS 参考求解但未通过论文门槛 | 8/9 | 八个论文设计均不得作为候选参考 |
| 已完成一轮项目独立候选工件生成 | 1/9 | 官方探针贴片已通过 `simulation_setup`、代码门禁和 124/124 冻结合同 |
| 已通过完整验证的独立自动生成候选 | 1/9 | 官方探针贴片已通过 G0-G5，同版本 S11 三项比较全部达标 |
...
也全部未通过其完整 VSWR 门槛。Ibrahim 38 GHz 案例的基线与两轮受控假设研究同样完成
真实求解并收敛，但基线和 8 个完成的变体均没有形成 -10 dB 频带。Khan 双频案例又
利用 Figure 2 修正了 V1 的圆环拓扑误读；V2 和 3 个端口/边界变体全部收敛，但仍未同时
复现两组论文谐振与完整频带。二者均被严格判为 `solved/fail-paper-gate`。**
...
| 4 | El-Gendy 5.25 GHz Wi-Fi 贴片 | 通过 | 通过 | 通过 | **未通过：21 个假设变体均收敛，最佳最坏点 -8.492 dB** | 按策略关闭 | 按策略关闭 |
...
| 8 | Ibrahim 38 GHz 单元 Antenna 3 | 通过 | 通过 | 通过 | **未通过：基线及 8 个收敛变体均无 -10 dB 频带** | 按策略关闭 | 按策略关闭 |
| 9 | Khan 28/38 GHz 双频单元 | 通过 | 通过 | 通过 | **未通过：V2 及 3 个收敛变体均未同时复现两个谐振和完整频带** | 按策略关闭 | 按策略关闭 |

两个 Yeo 贴片于 2026-08-26 在当前本地 AEDT 2025.1/PyAEDT 0.26.3 会话完成求解，因此
不再标为许可证阻塞。它们的“未通过”只针对冻结的论文电磁门槛：不能被提升为候选
比较基线。Wi-Fi 的假设变体、Kaur 三案例以及 Ibrahim 的两轮假设研究也已求解但未通过
各自完整门槛。Khan V2 及其 3 个受控变体也已求解并收敛但未通过双频门槛。至此 9 个
参考设计均已求解：1 个通过，8 个论文翻译失败；失败不等于
论文几何本身错误，而是说明当前冻结证据与明确标注的工程假设不足以重现论文响应。
...
- 2026-08-28 又完成 11 组 V2 多假设交互试验，11/11 均收敛、0/11 通过。最佳组合把
  完整目标频带最坏点改善到 -8.4920 dB，仍比 -10 dB 门槛差 1.5080 dB。
...
### 5.7 Ibrahim 2023 38 GHz 单元 Antenna 3

- 复现范围严格限定为 Figure 4 的单个 `Antenna 3`，不混入四端口 MIMO、解耦结构或 FSS。
- 论文明确参数包括 `12 x 12 x 0.203 mm`、`er=3.55`、圆形辐射体直径 `4.94 mm`、
  `W1=2.2`、`L1=2.45`、`L2=2.35`、`Wf=0.4`、`Lf=7.0` 和 `Lg=7.7 mm`。
- 论文目标冻结为约 38 GHz、最低 S11 约 -30 dB，以及 36.5-39.5 GHz 的 -10 dB 频带。
- Project8 中的基线在第 6 个自适应 pass 收敛，最终 `Delta S=0.01816`；最低点为
  41.47 GHz / -5.2409 dB，38 GHz 点为 -4.2186 dB，没有 -10 dB 频带。
- V1 只改变论文未披露的端口、导体厚度和辐射边界距离：3 个变体完成并收敛，2 个有限
  厚度导体变体因切触实体无法可靠合并而在建模阶段失败；0 个通过论文门槛。
- V2 保持所有可见论文尺寸不变，只改变馈线与圆盘之间隐藏的 CAD 布尔重叠。5/5 个变体
  均完成并收敛，最终统一后的导体几何等价，曲线在数值精度内一致；最低点约为
  41.10 GHz / -5.7676 dB，仍无 -10 dB 频带。
- 因此 G3 失败，G4/G5 按策略关闭；不得通过调整论文明确尺寸来强行得到 38 GHz 曲线。

### 5.8 Khan 2024 28/38 GHz 双频单元

- 范围仅包含 Figure 1/2 的单个辐射单元，不混入后续双端口 MIMO 与超材料结构。
- 论文明确给出 5 x 9.2 x 0.787 mm Rogers RT5880 基板、`er=2.2`、
  `tan(delta)=0.0009`、表 1 的 22 个几何标签，以及 24.86-28.65 GHz 和
  36.24-40.82 GHz 两个目标频带。
- V1 将顶部圆底槽误读为独立圆环；该错误被保留为负证据。Figure 2 明确显示外层倒 U、
  内层 U 和顶部圆底槽双杆，因此 V2 只修正拓扑解释，不修改任何表 1 数值。
- V2 在第 3 个自适应 pass 收敛，最终 `Delta S=0.01387`；得到约
  37.80-42.17 GHz 的第二频带，但最低点偏移至 40.90 GHz / -20.0647 dB，低频响应也
  没有形成论文曲线中的独立谐振谷。
- 随后的 3 项 OAT 研究只改变内部集总端口或 1/4 mm 辐射边界距离，3/3 全部收敛、
  0 项通过。最佳 1 mm 边界方案的低频整段最坏点为 -12.1844 dB，而高频整段最坏点
  仍为 -9.0388 dB，且谐振位置仍偏右。
- 因此 G3 失败，G4/G5 关闭。下一版必须由作者 HFSS 工程、更清晰坐标或其他新来源证据
  支持，不能继续无依据调整表 1 尺寸。
...
| --- | --- |
| 正确性验证软件框架 | 通过离线软件验证 |
| 参数/材料/拓扑抽取合同 | 9 个参考设计均已冻结并通过静态检查 |
| HFSS 参考模型电磁正确性 | 9 个已求解：1 个通过、8 个论文设计未通过 G3 |
| 图像+文本自动生成候选正确性 | 官方探针贴片 1 个完整 G0-G5 案例通过；其余 8 个案例因 G3 失败未进入候选 |
...
> 八个论文案例已经获得可审计的阴性参考证据，当前都不能作为候选基线。
...
5. 冻结 Ibrahim 基线、V1/V2 两轮假设研究、收敛证据和全部 S11 哈希；在获得原始 HFSS
   工程、端口/边界细节或数值曲线之前，不再调整该案例的论文明确尺寸。
6. 冻结 Khan V1、Figure-2 修正 V2 和 3 项端口/边界研究。V2 虽明显改善拓扑一致性，
   仍未通过双频门槛；只有作者工程或新的坐标证据才能授权下一版解释。
7. 每个参考模型必须先通过论文门槛，随后冻结 `.aedt`、S11 CSV、报告和 SHA-256；只有
...
- [活动状态说明](CAMPAIGN.md)
- [9 个独立案例 Python 入口](cases/CASE_INDEX.md)
...
- [Ibrahim 38 GHz 基线与两轮假设研究摘要](ibrahim_38ghz_monopole/reference_data/hfss_reference_and_assumption_studies_2026_08_30.json)
- [Ibrahim 38 GHz 案例说明](ibrahim_38ghz_monopole/README.md)
- [Khan 28/38 GHz V1/V2 与假设研究摘要](khan_28_38ghz_monopole/reference_data/hfss_reference_and_assumption_study_2026_08_31.json)
- [Khan 28/38 GHz 案例说明](khan_28_38ghz_monopole/README.md)
